...
# Load configuration
load_config() {
   local config_file="$HOME/.config/polish-text/config"
   ...
   if [[ -z "$active_function" ]]; then
      if [[ "$openai_available" == true ]]; then
         ...
         echo "You can also create a config file at ~/.config/polish-text/config" >&2
         exit 1
      fi
   fi
...
ollama() {
   ...
   if ! command -v ollama >/dev/null 2>&1; then
      ...
      echo "Install Ollama or set OLLAMA_API_URL in ~/.config/polish-text/config" >&2
      exit 1
   fi
...
openai() {
   ...
   if [[ -z "${OPENAI_API_KEY:-}" ]]; then
      ...
      echo "Or add it to ~/.config/polish-text/config as: export OPENAI_API_KEY='your-key'" >&2
      exit 1
   fi
...
claude() {
   ...
   if [[ -z "${CLAUDE_API_KEY:-}" ]]; then
      ...
      echo "Or add it to ~/.config/polish-text/config as: export CLAUDE_API_KEY='your-key'" >&2
      exit 1
   fi
