#!/usr/bin/env bash
# ABOUTME: Configuration system tests for polish-text
...
mkdir -p "$HOME/.config/polish-text"
if [[ -d "$HOME/.config/polish-text" ]]; then
...
cat > "$HOME/.config/polish-text/config" <<EOF
export CLAUDE_API_KEY="test-claude-key"
openai_model="gpt-4"
ollama_model="llama2"
EOF

if [[ -f "$HOME/.config/polish-text/config" ]]; then
...
source "$HOME/.config/polish-text/config"
