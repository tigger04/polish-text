#!/usr/bin/env bash
# ABOUTME: Integration tests for polish-text
...
if [[ -f "./polish-text" && -f "./summarize-text-lib.sh" && -f "./LICENSE" ]]; then
...
mkdir -p "$TMP_DIR/test-config/.config/polish-text"
if [[ -d "$TMP_DIR/test-config/.config/polish-text" ]]; then
...
echo "This is a test document for text polishing." > "$TMP_DIR/test.txt"
